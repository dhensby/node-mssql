...
// Connection extends a typed EventEmitter with a single-event fixed set:
interface ConnectionEvents {
  close: [{ reason: 'user' | 'remote' | 'error' | 'reset'; error?: MssqlError }]
}
```

`Connection extends TypedEventEmitter<ConnectionEvents>` — full node `EventEmitter` API (`on`, `once`, `off`, `addListener`, `removeAllListeners`, etc.), typed to the single event above. The decision is *what events exist*, not *which EventEmitter methods are allowed*; once we commit to the node EventEmitter pattern, the standard surface is more useful than fighting it.
...
- `close` — connection has finished its lifetime. Reason disambiguates normal teardown (`'user'`), server-initiated close (`'remote'`), fatal error (`'error'`), and pool-triggered `sp_reset_connection` cycle (`'reset'`). A connection that closes due to a fatal error carries the error in the payload; there is no separate `error` event.

Everything else — per-request info/print/envChange, connection-scoped failures that are not a close (they *are* a close, with `reason: 'error'`), auth events, keepalive traffic — lives in `ResultEvent` (for per-request data) or `diagnostics_channel` (for observability). No `'error'` event means no node `EventEmitter` no-listener-crash hazard: pooled idle connections cannot take the process down by emitting unhandled `'error'`.
