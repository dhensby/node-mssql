...
describe('EventBridge — termination', () => {
	...
	test('fail() (request completion-callback error) → events.on iterator throws', async () => {
		...
		const iter = on(bridge, 'data', { close: ['end'] }) as AsyncIterableIterator<[ResultEvent]>;
		// tedious delivers execSql / execSqlBatch errors via the request's
		// completion callback (NOT the 'error' event tested above);
		// connection.ts forwards that to fail().
		bridge.fail(new Error('duplicate key'));
		await assert.rejects(() => collect(iter), /duplicate key/);
	});

	test('fail() then requestCompleted → still throws (error wins over the natural end)', async () => {
		const { bridge, request } = buildBridge();
		const iter = on(bridge, 'data', { close: ['end'] }) as AsyncIterableIterator<[ResultEvent]>;
		// Real ordering: tedious calls the completion callback (→ fail) and
		// THEN emits requestCompleted (→ 'end'). The recorded error must win,
		// not be swallowed by the close.
		bridge.fail(new Error('constraint violation'));
		request.fireRequestCompleted();
		await assert.rejects(() => collect(iter), /constraint violation/);
	});
});
