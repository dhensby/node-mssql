	...
	implements Connection
{
	...
	execute(req: ExecuteRequest, signal?: AbortSignal): AsyncIterable<ResultEvent> {
		...
		// fires only for `prepare`). Forward it to the bridge so the
		// iterator throws; `requestCompleted` (emitted right after) still
		// drives the natural `'end'`. The `bridge` const is initialised on
		// the next line and is always defined by the time this async
		// callback fires.
		const request = new TediousRequest(req.sql, (err) => {
			if (err !== undefined && err !== null) bridge.fail(err);
		});
