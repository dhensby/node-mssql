...
In either recoverable case, the consumer never sees a `ConnectionError` from the path, and `mssql:connection:reset` with `stage: 'on-acquire'` is emitted from the adapter each time the hook runs (whether or not it ultimately succeeded — see [ADR-0014](0014-diagnostics.md)).
...
A `DriverError` in production is a bug report: the driver should have classified it into a more specific subclass. Presence is monitored via `mssql:connection:driver-error` ([ADR-0014](0014-diagnostics.md)).
