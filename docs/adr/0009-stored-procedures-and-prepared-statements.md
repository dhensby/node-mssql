...
- **Prepared statements** have the same input typing, but their lifecycle is materially different: a prepared statement creates a server-side handle that is **connection-pinned** and must be explicitly released. A first-execute / re-execute / dispose lifecycle has to be modelled. (On TDS this corresponds to the `sp_prepare`/`sp_execute`/`sp_unprepare` system-procedure cycle and on ODBC to `SQLPrepare`/`SQLExecute`/`SQLFreeHandle`, but the kernel addresses prepared statements through the abstract lifecycle, not the wire mechanism — see [ADR-0010](0010-driver-port.md), "Kernel describes intent; drivers translate to wire".)
...
// Prepared statement: text + typed slots produce a server-side prepared handle.
...
Prepared statements differ from procedures in one respect: the server-side handle is **connection-pinned** and must be **explicitly released**. `sql.prepare()` returns a `PreparedStatement` that is simultaneously a callable template *and* an `AsyncDisposable`:

```ts
await using stmt = sql.prepare('select * from users where id = @id')
  .input('id', types.Int)

// First .bind().terminal() call: prepares the statement, pins a connection, caches the handle.
const alice = await stmt.bind({ id: 1 }).all()

// Subsequent calls: reuse the handle on the same pinned connection.
const bob = await stmt.bind({ id: 2 }).all()

// End of `using` scope: handle is released, connection returns to the pool.
```

Semantics:

- **First `.bind().terminal()` triggers preparation.** The prepared statement is lazy in the same way `Query` is ([ADR-0008](0008-query-lifecycle-and-disposal.md)) — `sql.prepare(text).input(...)` builds the template without touching the server. The server handle is created on the first execution, at which point a connection is acquired and pinned for the rest of the `PreparedStatement`'s lifetime.
- **Subsequent executions reuse the handle.** The cached handle stays alive for the whole scope; further `.bind().terminal()` calls reuse it on the pinned connection without re-preparing.
- **`.dispose()` releases the handle and the pinned connection.** Uniform with `ReservedConn` disposal ([ADR-0008](0008-query-lifecycle-and-disposal.md)) — the connection goes back to the pool, the server handle is freed, and any further `.bind()` on the disposed statement throws.
...
- Prepared statements add a **connection-pinned lifecycle** the user opts into: prepare on first execute, reuse the handle on subsequent calls, release on dispose. Connection pinning is the cost of the better plan-cache behaviour a server-side prepared handle gives — users who do not need that reach for the bare `sql` tag, which uses parameterised execution without pinning.
