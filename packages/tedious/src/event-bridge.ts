...
export class EventBridge extends EventEmitter<BridgeEvents> {
	...
	// Surface a request error as an iterator throw. tedious delivers
	// SQL / request errors for `execSql` / `execSqlBatch` through the
	// request's *completion callback* — NOT an `'error'` event (that fires
	// only for `prepare`) — so `execute()` forwards the callback's error
	// here. Buffered `'data'` events still drain first (events.on's
	// contract); the throw then propagates. `requestCompleted` ('end')
	// fires right after, but the recorded error takes priority.
	fail(err: Error): void {
		this.emit('error', err);
	}
