...
`.raw()` is a **view toggle**, not a cardinality terminal. It returns a new `Query<R[]>` whose rows arrive as positional tuples instead of objects, in the order reported by `.columns()`. Every other terminal — `.all()`, `.iterate()`, the default `await` / `for await`, `.run()`, `.result()`, `.rowsets()`, `.meta()` — works identically on the raw-mode query, so `await q.raw()`, `for await (const row of q.raw())`, and `await q.raw().rowsets()` all make sense. The tuple element type defaults to `unknown[]`; callers who know the column order can narrow with a tuple type argument, `q.raw<[number, string, Date]>()`.
...
interface QueryMeta<O = Record<string, never>> {
  ...
  completed: boolean                     // true if the stream drained naturally
}
```
...
- On non-natural exit (`AbortSignal`, iterator `return()`, early termination, error mid-stream), the stream terminates and meta state is populated with `completed: false`. Inside the catch block (or after a `for await` `break`), `q.meta()` returns synchronously with whatever trailer data arrived before the exit — useful for "what info messages did I get before the abort?" debugging. The consumer already has the catch-site error (or knows they broke) for the *what happened* question; meta exists to expose the trailer data, not to re-state outcome the caller can see from control flow.
