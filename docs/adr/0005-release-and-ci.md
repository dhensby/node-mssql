...
**Cross-package atomicity.** A fix that touches multiple sub-packages produces one release PR per affected sub-package plus a meta release PR. release-please opens all of these in the **same run** — the meta PR's pins are *prospective*: it pins to the version each sub-package *will have* when its PR merges, computed deterministically from the conventional-commit log. The `node-workspace` plugin re-renders the meta PR whenever the rendered sub-package versions change (more commits land, an existing PR is updated), so by the time a maintainer reviews the meta PR its pins reflect the latest computed versions of every affected sub-package.

The merge order for an atomic cross-package fix is therefore:

1. Merge each affected sub-package's release PR and **wait for its npm-publish job to complete** (order across sub-packages does not matter; each publishes independently).
2. Once every affected sub-package is reachable on npm, merge the meta release PR — its pins resolve cleanly and the meta publishes with the new sub-package versions together.

A maintainer who merges the meta release PR before the sub-package PRs have merged and published would publish a meta version whose `dependencies` reference versions not yet on npm. The pins are coherent (release-please rendered them to a consistent set), but `npm install mssql@<that version>` would fail to resolve the deps until the sub-package PRs catch up — a broken-release window, not a mixed-pins state. This is operator discipline, not tooling-enforced. The mitigation is a documented release procedure (in `RELEASING.md`) plus a meta-release PR template that surfaces the currently-open sub-package release PRs and asks the maintainer to confirm they have all merged *and published* before merging the meta. A stronger CI gate — block the meta publish job if any other release-please PR is open — is deferred until real-world experience shows the procedure alone is insufficient.
...
A practical caveat: Dependabot reads its config from the repository's **default branch** (currently `master`). The workspace-aware config sitting on `next-major` is therefore inert during the pre-release phase — Dependabot keeps using `master`'s v12 config and only opens PRs against the v12 layout. This is acceptable: a pre-release branch does not need automated dependency updates, and any updates `next-major` wants during the rewrite are done by hand. When v13 merges into `master`, the workspace-aware config travels with the merge and Dependabot starts opening PRs against the workspace packages from that point on. No separate sync PR to `master` is required during the rewrite.
...
- **Cross-package fixes land atomically on the meta when the merge order is followed.** A fix touching multiple sub-packages produces one release PR per affected sub-package plus a meta PR, all opened in the same release-please run with prospective pins on the meta. Merging all sub-package PRs first (and waiting for their npm-publish jobs to complete) before merging the meta results in a single meta version that pins the new fix in every affected sub-package. The "merge order" risk (a meta PR merged before its sub-packages publish would ship pins to versions not yet on npm, breaking `npm install` until the sub-packages catch up) is mitigated by documented procedure and a PR-template prompt, not tooling-enforced — see "Cross-package atomicity" above.
...
- **Dependabot is inert against the `next-major` workspace until v13 merges to `master`.** Acceptable: a pre-release branch does not need automated dep updates, and manual control during the rewrite is the right tradeoff. When v13 lands on `master`, the workspace-aware config travels with the merge and updates flow automatically from then on — no separate sync PR required.
