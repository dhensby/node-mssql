...
// ─── Per-rowset columns (streamed form) ─────────────────────────────────────
//
// Each yielded rowset carries its own `.columns()` — distinct from the
// Query-level `q.columns()`, which is locked to the first rowset
// (ADR-0007). This is the path that makes `.raw()` over multiple
// rowsets usable: positional tuples beyond the first rowset are
// meaningless without the per-rowset column names.

describe('Query.rowsets() — per-rowset columns()', () => {
	test('each rowset reports its own columns', async () => {
		const { runner } = makeFakeRunner([
			{ kind: 'metadata', columns: [{ name: 'id' }, { name: 'name' }] },
			{ kind: 'row', values: [1, 'alice'] },
			{ kind: 'rowsetEnd', rowsAffected: 1 },
		...
		const q = new Query({ runner, request: stmt('SELECT id, name FROM u; SELECT orderId, amount FROM o') });
		const colsSeen: { name: string }[][] = [];
		for await (const rs of q.rowsets()) {
			colsSeen.push((await rs.columns()).map((c) => ({ name: c.name })));
			for await (const _row of rs) { /* drain */ }
		}
		assert.deepEqual(colsSeen, [
			[{ name: 'id' }, { name: 'name' }],
			[{ name: 'orderId' }, { name: 'amount' }],
		]);
	});

	test('columns() resolves BEFORE any rows are pulled from the rowset', async () => {
		const { runner } = makeFakeRunner([
			{ kind: 'metadata', columns: [{ name: 'n' }] },
			{ kind: 'row', values: [1] },
			{ kind: 'rowsetEnd', rowsAffected: 1 },
		...
		const q = new Query({ runner, request: stmt('SELECT n') });
		for await (const rs of q.rowsets()) {
			// Read columns first, without having pulled a single row.
			const cols = await rs.columns();
	...
	test('columns() is callable after the rowset has been drained, returning the same metadata', async () => {
		const { runner } = makeFakeRunner([
			{ kind: 'metadata', columns: [{ name: 'n' }] },
			{ kind: 'row', values: [1] },
			{ kind: 'rowsetEnd', rowsAffected: 1 },
		...
		const q = new Query({ runner, request: stmt('SELECT n') });
		for await (const rs of q.rowsets()) {
			for await (const _row of rs) { /* drain first */ }
			const cols = await rs.columns();
			assert.deepEqual(cols, [{ name: 'n' }]);
		}
	});

	test('columns() is callable multiple times and returns equal metadata each call', async () => {
		const { runner } = makeFakeRunner([
			{ kind: 'metadata', columns: [{ name: 'n' }] },
			{ kind: 'row', values: [1] },
			{ kind: 'rowsetEnd', rowsAffected: 1 },
		...
		const q = new Query({ runner, request: stmt('SELECT n') });
		for await (const rs of q.rowsets()) {
			const a = await rs.columns();
			const b = await rs.columns();
			assert.deepEqual(a, [{ name: 'n' }]);
			assert.deepEqual(b, [{ name: 'n' }]);
		}
	});

	test('raw mode: columns() maps positional tuple slots to names per rowset', async () => {
		// The headline use case — raw tuples beyond the first rowset are
		// meaningless without the per-rowset column names.
		const { runner } = makeFakeRunner([
			{ kind: 'metadata', columns: [{ name: 'a' }, { name: 'b' }] },
			{ kind: 'row', values: [1, 2] },
			{ kind: 'rowsetEnd', rowsAffected: 1 },
			{ kind: 'metadata', columns: [{ name: 'x' }, { name: 'y' }, { name: 'z' }] },
			{ kind: 'row', values: [7, 8, 9] },
			{ kind: 'rowsetEnd', rowsAffected: 1 },
			{ kind: 'done' },
		]);
		const q = new Query({ runner, request: stmt('SELECT a, b; SELECT x, y, z') }).raw();
		...
		for await (const rs of q.rowsets()) {
			...
			for await (const tuple of rs) {
				...
				cols.forEach((c, i) => { obj[c.name] = t[i]; });
				mapped.push(obj);
			}
		}
	...
	test('columns() reflects the rowset reached after an inner-break drain', async () => {
		// Break out of rowset 1 early; rowset 2's columns() must still be
		// that rowset's own columns, proving the accessor tracks the
		// yielded rowset, not the first.
		const { runner } = makeFakeRunner([
			{ kind: 'metadata', columns: [{ name: 'n' }] },
			{ kind: 'row', values: [1] },
			{ kind: 'row', values: [2] },
		...
		for await (const rs of q.rowsets()) {
			const cols = await rs.columns();
			colsSeen.push(cols.map((c) => c.name));
			for await (const _row of rs) {
				if (outerIndex === 0) break;  // break rowset 1 after first row
			}
			outerIndex++;
		}
		assert.deepEqual(colsSeen, [['n'], ['m']]);
	});
});
