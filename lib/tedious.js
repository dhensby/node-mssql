...
class ConnectionPool extends base.ConnectionPool {
  _poolCreate () {
    return new base.Promise((_resolve, _reject) => {
      // define single use resolve / reject functions - this prevents us from resolving the same
      // connection repeatedly which may sometimes happen if multiple events are fired during
      // a connections lifetime
      let resolve = (val) => {
        _resolve(val)
        resolve = () => {}
        reject = () => {}
      ...
      tedious.on('done', evt => {
        reject(
          new base.ConnectionError(
            'Connection ended unexpectedly before establishing a connection'
          )
        )
      })
