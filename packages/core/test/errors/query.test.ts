...
test('QueryError — carries TDS fields verbatim', () => {
...
test('QueryError — optional fields absent when not provided', () => {
...
test('ConstraintError — extends QueryError with kind + constraintName', () => {
...
test('constraintKindFromNumber — unique maps 2627 and 2601', () => {
	assert.equal(constraintKindFromNumber(2627), 'unique')
	assert.equal(constraintKindFromNumber(2601), 'unique')
})

test('constraintKindFromNumber — 547 defaults to foreignKey', () => {
	assert.equal(constraintKindFromNumber(547), 'foreignKey')
})

test('constraintKindFromNumber — 547 with CHECK hint returns check', () => {
	assert.equal(
...
test('constraintKindFromNumber — notNull and default', () => {
...
test('constraintKindFromNumber — unknown number returns undefined', () => {
	assert.equal(constraintKindFromNumber(1), undefined)
	assert.equal(constraintKindFromNumber(99999), undefined)
})

test('MultipleRowsetsError — extends MssqlError, name set', () => {
