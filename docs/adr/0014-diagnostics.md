...
| Emitter | Channels |
|---|---|
| Core (kernel) | `mssql:query`, `mssql:execute`, `mssql:prepare`, `mssql:transaction:begin`, `mssql:transaction:commit`, `mssql:transaction:rollback`, `mssql:transaction:savepoint:begin`, `mssql:transaction:savepoint:rollback`, `mssql:transaction:savepoint:release`, `mssql:request:info`, `mssql:request:print`, `mssql:request:env-change`, `mssql:client:state-change` |
...
| `mssql:transaction:savepoint:begin` | `{ connectionId, transactionId, savepointId, name }` — a savepoint created via `tx.savepoint()` (`SAVE TRANSACTION <name>` on the wire), pushed onto the transaction's stack ([ADR-0006](0006-queryable-api.md), "Savepoints"). |
| `mssql:transaction:savepoint:rollback` | `{ connectionId, transactionId, savepointId }` — a roll-back to a savepoint (`ROLLBACK TRANSACTION <name>` on the wire), via `sp.rollback()` or `tx.rollbackSavepoint()` — distinct from the full `mssql:transaction:rollback` |
| `mssql:transaction:savepoint:release` | `{ connectionId, transactionId, savepointId }` — a savepoint released: dropped from the transaction's stack with its work kept, via `sp.release()` / `tx.releaseSavepoint()` or the `await using` disposal of an un-settled savepoint handle. **Application-layer marker only, no wire round-trip** — SQL Server has no `RELEASE SAVEPOINT` verb (see [ADR-0008](0008-query-lifecycle-and-disposal.md)). There is no `savepoint:commit`: a savepoint has no individual commit; its kept work is covered by `mssql:transaction:commit` when the transaction commits. |
...
interface ChannelParam {
  type: SqlType        // SqlType value — types.Int(), types.VarChar(255), types.Bit(), etc.
...
Both `type` and `value` are always populated. The driver requires a SqlType for every parameter to encode it on the wire, so by the time we publish the channel — after request build, before dispatch — a type is known for every entry. Procedure builder users supply it explicitly (`.input('id', types.Int(), 123)`); tagged-template and `sql.unsafe()` users get the same JS-value-to-SqlType inference the driver applies for the wire format (number → Int, string → NVarChar('max'), Buffer → VarBinary('max'), Date → DateTime2, etc.). `value` is the user's JS value unchanged; the driver's wire-level coercion (e.g. `true` → bit `1`) is not applied at the channel layer — observability cares about what the user wrote, not the wire shape.
