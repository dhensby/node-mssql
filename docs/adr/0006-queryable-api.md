...
The `sql` value returned from `createClient` is a tagged-template callable. Pool, reserved connection, and transaction are all the same shape — the callable, bound to a scope. `Promise.all` always works; whichever scope is serialised internally (a reserved connection, a transaction) queues parallel callers on the underlying connection so the user never sees a serialisation error.
...
| `.result<T>()` | `{ rows: T[]; meta: QueryMeta<O> }` | — | Buffers rows *and* returns trailer in one shape. Inline-friendly for DML with `OUTPUT` and procs whose `output` parameters are wanted alongside rows; equivalent to `await q.all()` + `q.meta()` but in one terminal call |
...
The streamed shape is `AsyncIterable<AsyncIterable<Tuple[number]>>`. **TypeScript cannot per-iteration-narrow against the tuple positions** — each yielded rowset is typed as the union of the tuple's elements (`AsyncIterable<User | Order>`), not as the position-specific type. Runtime emits rowsets in source-SQL order, and row order within a rowset is as the server emits it, but the type system doesn't reflect the position. Users who need per-rowset typing either:

- Use the **awaited form** `const [users, orders] = await q.rowsets<[User, Order]>()` — this narrows correctly as a tuple because the whole structure exists before TS sees it. Best when materialisation is acceptable.
- Discriminate inside the `for await` loop by rowset index (`let i = 0; for await (...) { if (i === 0) { /* cast */ } i++ }`) — workable but ugly.

A future `q.rowsetsTagged<[User, Order]>()` returning `{ index: 0; rows: AsyncIterable<User> } | { index: 1; rows: AsyncIterable<Order> }` could cover the streamed-and-typed case via discriminated-union narrowing. Out of scope for v13.0; non-breaking to add if real demand emerges.
...
All scopes return the same callable shape, and every scope factory shares one signature pattern: each returns a builder that is awaitable (resolving to an `AsyncDisposable` handle) and carries chainable configuration methods. The handle, once awaited, *is* the callable tag plus the scope's lifecycle methods:
...
Each layer **extends** the callable rather than replacing it. `ReservedConn` and `Transaction` are both callable tags (so the queryable API works directly on them) plus the lifecycle methods specific to that scope:

| Scope | Adds beyond the callable | Disposal default |
|---|---|---|
| `ReservedConn` | `.transaction()` | release to pool |
| `Transaction` | `.commit()`, `.rollback()`, `.savepoint()`, `.rollbackSavepoint()`, `.releaseSavepoint()` | rollback if not committed |

`.transaction()` opens a transaction on the pool-bound `sql` or a `ReservedConn`. Within a transaction, partial rollback is done with **savepoints** — `tx.savepoint()` / `tx.rollbackSavepoint()` — not with nested transaction objects (see "Savepoints" below). A unified, re-entrant model where `.transaction()` *also* nests on a `Transaction` — making pool / reserved connection / transaction / savepoint a single interchangeable scope type — is attractive for dependency-injection (e.g. handing a test its transaction as "the client"), but it is **deferred**; see "Alternatives considered".

Configuration is by chained builder methods, not an options-bag — `.signal(s)` on every scope factory, plus `.isolationLevel(level)` on `sql.transaction()`. The shape mirrors `Query<T>`: each factory returns a builder that is awaitable (triggering the scope creation on `await`, resolving to the scope handle) and carries the chainable configuration. There is no `opts` argument; the chain is the configuration surface.
...
Supported levels: `'read uncommitted'`, `'read committed'`, `'repeatable read'`, `'snapshot'`, `'serializable'` — strict lowercase. Other casings (e.g. .NET-style `'ReadCommitted'`) throw `TypeError` at the call site: the TypeScript literal type rejects them at compile time, and a runtime validator covers JavaScript callers and dynamically-built strings. SNAPSHOT requires `ALLOW_SNAPSHOT_ISOLATION ON` at the database level; the library does not validate this — SQL Server raises an error if it is off.
...
**Savepoints inherit.** Savepoints share the transaction's isolation level — the protocol has no per-savepoint override. So `.isolationLevel()` lives on `sql.transaction()` only; there is nothing to set on `tx.savepoint()` (see "Savepoints" below).

### Savepoints — transaction-managed bookmarks

Within a transaction, partial rollback is done with **savepoints**: named bookmarks the transaction records on a LIFO stack. A savepoint is *not* a transaction — it has no independent commit. There are two wire operations — creating a mark (`SAVE TRANSACTION <name>`) and rolling back to one (`ROLLBACK TRANSACTION <name>`) — plus one **application-layer** operation, **release**: dropping a mark from the stack while keeping its work, so a later rollback goes further back. SQL Server has no `RELEASE SAVEPOINT` verb (unlike Postgres / SQLite), so release is purely stack bookkeeping — no round-trip. The **transaction owns the stack**, so it can orchestrate rollbacks and releases even when the caller doesn't hold a savepoint handle.

```ts
await using tx = await conn.transaction()
await tx`INSERT …`.run()

const sp = await tx.savepoint()         // SAVE TRANSACTION — push a mark
await tx`INSERT …`.run()
await sp.release()                       // done with sp: keep its work, drop the mark (no wire op)

const sp2 = await tx.savepoint()
await tx`INSERT …`.run()
await sp2.rollback()                     // ROLLBACK TRANSACTION sp2 — undo the work after sp2
// …or, when the handle isn't in scope (e.g. across test-fixture hooks):
await tx.rollbackSavepoint()             // roll back to the most-recent live mark

await tx.commit()                        // the only durable commit; keeps everything not rolled back
```

**Queries run on the transaction.** Savepoints are markers, not scopes — you keep issuing `` tx`…` `` and use savepoints only to bound what a later rollback undoes. `tx.savepoint()` returns a small handle — `{ name, rollback(), release(), [Symbol.asyncDispose] }` — for callers who want to target a specific mark; `tx.rollbackSavepoint(name?)` and `tx.releaseSavepoint(name?)` are the transaction-orchestrated paths (the latest mark, or a named one). The handle-less paths are what make savepoints usable where the creating scope isn't in reach — e.g. test fixtures that bracket each `describe` / `beforeEach` with `tx.savepoint()` and `tx.rollbackSavepoint()` / `tx.releaseSavepoint()` without threading handles through.

**Release vs rollback.** Both pop the mark (and any marks created after it) off the stack; they differ only in what happens to the work since the mark. `release()` **keeps** it ("I'll never come back to this point — let the next rollback go further back"); `rollback()` **discards** it (`ROLLBACK TRANSACTION`). Release is the explicit "I'm done with this bookmark" — it returns the stack to where it was so a later `tx.rollbackSavepoint()` can't accidentally land on a mark you're finished with.

**Kept by default.** A savepoint's work is part of the transaction; it commits when the transaction commits unless you explicitly **roll back** to (or past) the mark. Release and disposal both *keep* the work — `await using sp`'s disposal releases the mark (drops it from the stack) if you haven't already rolled it back or released it. This is the right default *because* a savepoint is a bookmark, not a transaction: a `Transaction` rolls back on dispose if you don't commit it (you must commit a transaction to keep its work), whereas a savepoint has nothing to commit — disposing it drops the bookmark and leaves its work in the transaction. SQL Server's lack of a `RELEASE` verb stops being a wart and becomes the model.

**Rolling back to a mark pops the stack to it.** `ROLLBACK TRANSACTION X` discards everything done after savepoint X — *including any savepoints created after X*. So rolling back to X (via `sp.rollback()` or `tx.rollbackSavepoint(...)`) removes X and every later mark from the stack; their handles are spent. Rolling back to an *earlier* mark therefore invalidates the later ones — which is correct, and is exactly the consistency the transaction-owned stack buys: a later savepoint's handle is never left believing it is live after an earlier rollback discarded its mark on the server. A spent savepoint handle's `.rollback()` throws `TypeError`; its `await using` disposal is a forgiving no-op.

**The transaction clears the stack on settle.** `tx.commit()` commits the whole transaction (every un-rolled-back mark's work included); `tx.rollback()` discards all of it. Both clear the savepoint stack, leaving any outstanding handle spent (explicit `.rollback()` throws, disposal no-ops). Crucially, settling the transaction is **one wire operation** regardless of how many savepoints are open: there is no per-savepoint settlement ceremony, and `tx.rollback()` in particular does **not** issue a cascade of `ROLLBACK TRANSACTION <mark>` round-trips first — a single `ROLLBACK` discards everything. (This is why savepoints are managed by the transaction rather than modelled as nested transactions you must individually settle — that ceremony would be wasted wire traffic for an unrecoverable transaction, and a lost child handle could otherwise block the parent's commit with no way to settle it.)

**Settling is idempotent, and control ops serialise with queries.** The first call to settle is recorded synchronously and held; every later `commit()`, `rollback()`, or disposal awaits that same outcome rather than issuing a second wire op. So parallel `commit()`s collapse to one `COMMIT`, and an un-awaited `commit()` followed by `await using` disposal is *not* turned into a rollback — disposal sees the in-flight settle and waits for it ("rollback if not committed" means *and not committing*). The wire control ops themselves — `BEGIN` / `SAVE` / `ROLLBACK TO` / `COMMIT` / `ROLLBACK` — run through the same single-in-flight queue as the transaction's queries (TDS serves one request per connection), so a control op never overlaps a query or another control op: two `tx.savepoint()` calls raced with `Promise.all` create two ordered marks instead of colliding (no `EREQINPROG`), and a `commit()` issued while a query is still running waits for it. This is the transaction-scope side of the "scopes serialise internally; `Promise.all` always works" guarantee above — it covers control ops, not only queries.

**Names** are library-generated via the shared object-id scheme (`savepointName()`, [ADR-0016](0016-object-id-format.md)) — `sp_<process-tag>_<counter>`, a valid ≤32-character SQL Server savepoint identifier. Because the name travels to SQL Server as a savepoint identifier it is SQL-safe by construction and is *not* subject to a custom `idGenerator` override (see ADR-0016, "Wire identifiers are excepted"). `sp.name` exposes it for `tx.rollbackSavepoint(name)` and for diagnostics correlation.
...
- Is available on every scope (the pool-bound `sql`, a `ReservedConn`, a `Transaction`) — it is a method on the same callable, not a top-level one.
...
Native sql-fragment composition — composing safe `` sql`...` `` fragments into larger queries without round-tripping through a raw string — is **out of scope for this library, by design**. Dynamic SQL belongs in dedicated query-builder libraries (Kysely, Drizzle, Slonik, et al.), which solve fragment composition properly with mature type-level safety, parameter merging, and ecosystem-specific helpers. This library's value proposition is *safe execution* of SQL, not *construction* of it; conflating the two would duplicate work the query-builder ecosystem already does well, expand the API surface significantly, and pull this library in a direction it deliberately is not.

`sql.unsafe(text, params)` is the **permanent integration point** for query-builder output. A user who needs dynamic SQL picks a builder, lets it produce a `{ text, params }` pair, and feeds that through `sql.unsafe`. Future effort goes toward making that integration safer and more ergonomic — type-level annotations on builder results, branded fragment types we accept from known builders, dedicated channels for builder telemetry — not toward building native fragment composition here.
...
`sql.transaction()` pins one connection for the scope duration — so temp tables are a natural fit inside a transaction (and inside its savepoints, which share that connection).
...
The connection's default reset-on-release (`Connection.reset()`, called by the pool adapter inside its release path) restores the connection to its initial-acquired state — temp tables cleared, `SET` settings reset, isolation level reset, and **database context restored**. A pool-bound `` sql`USE other_db`.run() `` on one acquirer does not leak into the next acquirer's view: the driver tracks current database via TDS `ENVCHANGE` and reverts on reset when it differs from the initial.
...
- Consumers no longer carry context about "what kind of thing is this" — a function taking `sql: Queryable` can be called with the pool, a reserved connection, or a transaction (top-level or nested) with identical behaviour.
...
- Each scope factory (`sql.acquire()`, `sql.transaction()`, and the procedure / prepared-statement templates) returns an `AsyncDisposable` handle that adds its scope-specific methods on top of the same callable tag shape. Consumption is uniform: `await using` for cleanup, explicit `.commit()` / `.rollback()` (or `.release()` for a reserved connection) on the happy path; within a transaction, `tx.savepoint()` / `tx.rollbackSavepoint()` bound partial rollbacks. There is no callback wrapper form — `await using` covers the automatic-cleanup case the wrapper would have given, and one shape is easier to learn than two.
- Session-scoped state (temp tables, `SET` settings, database context) works the way it does in every pooled SQL client: safe inside `sql.acquire()` / `sql.transaction()` (and its savepoints), not safe across bare pool-bound `sql` calls. The default `Connection.reset()` on release fully restores the connection to its initial-acquired state — including the database context, which the underlying TDS reset primitive does not cover but the driver does.

## Alternatives considered

**Re-entrant `.transaction()` — a single interchangeable scope type (deferred).** A unified model where `.transaction()` is re-entrant — calling it on a `Transaction` opens a nested transaction of the *same type*, implemented as a savepoint — so that pool, reserved connection, transaction, and savepoint are one interchangeable callable scope. The appeal is real: a function (or a test harness) can take "a scope" and call `.transaction()` on it without knowing whether it holds a pool, a connection, or an enclosing transaction — e.g. injecting a transaction as the application's "client" so a suite's writes roll back transparently. It was prototyped and rejected **for v13.0, deferred as future work**, because it forces the awkward parts of the tree-vs-stack impedance mismatch into the core: nested transactions present a tree, but SQL Server savepoints are a flat stack, so siblings and out-of-creation-order settlement need either an artificial "one open child" constraint or a cascade-tracking stack to stay consistent; and modelling each savepoint as a transaction (with its own commit/rollback/disposal) imposes settlement ceremony on a primitive that has no `RELEASE` and where a lost child handle can block the parent's commit. The transaction-managed savepoint model (above) is the simpler, honest base: savepoints are bookmarks the transaction owns, kept-by-default, settled wholesale when the transaction settles. The re-entrant unification can be layered on top later once the base is proven, without changing the savepoint semantics underneath.
...
**Ship `.one()` / `.get()` as v13.0 terminals.** Considered — every major client ships at least one of them. Deferred to v13.1+ as non-breaking additions. `.one()` is a thin wrapper around `(await q.all())[0] ?? null`; the inline form is one line and makes the "or what on empty?" question explicit at the call site rather than buried in terminal semantics. Better-sqlite3's `.expand()` for joined-row namespacing is a separate future consideration, also v13.1+. v13.0 commits to the cardinality-named terminal set (`.all()`, `.iterate()`, `.run()`, `.result()`, `.rowsets()`) plus the view-toggle (`.raw()`) and shape-introspection pair (`.columns()`, `.meta()`); everything else is additive.

**Defer `.result()` to v13.1+.** Earlier draft. Reversed: `.result()` ships in v13.0 because the inline-await DML-with-`OUTPUT` case is the most-cited migration friction from v12 (which always returned `{ recordset, rowsAffected, output, returnValue }`). Without `.result()`, that case forces three lines (`` const q = sql`…`; const rows = await q; const { rowsAffected } = q.meta() ``) for what reads naturally as a one-liner. The cost is one additional terminal in the v13.0 surface; the benefit is cleanly serving the migration window where v12-shaped expectations are at their strongest. `.all()` + `q.meta()` remains available for users who prefer it.
