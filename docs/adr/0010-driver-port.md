...
- **Surfacing decoded values without pre-emptive precision loss.** Precision-sensitive values — `decimal` / `numeric` / `money`, large integers — are surfaced in a lossless form (e.g. as strings), short-circuiting any narrowing the native driver does by default, so the type system's decode layer ([ADR-0019](0019-sql-type-system.md)) decides the final JS type (the default `number` parse, or a consumer's custom decode that keeps full fidelity). Complex types core parses itself — spatial `geometry` / `geography` — are surfaced as their raw wire bytes for core's dependency-free parser rather than pre-decoded.
