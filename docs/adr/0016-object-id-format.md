...
**`PROCESS_TAG`**: 8 hex characters derived from a 4-byte buffer filled by `globalThis.crypto.getRandomValues()`, generated **once** at core module load. One tag per process. 32 bits of entropy is enough to make collision across a few thousand concurrent processes vanishingly unlikely in practice, while keeping the tag short.
...
- Log lines become correlatable across objects in the same process (`conn_a3f2b19c_1` appears in the connect event, the query events that used it, and the release event) and distinguishable across processes (different `PROCESS_TAG`).
- IDs are short enough to fit in a log prefix without wrapping (~15 characters vs ~36 for a UUID).
- The counter is not durable across restarts — `req_a3f2b19c_47` from yesterday and `req_a3f2b19c_47` from today are different requests. This is fine: `PROCESS_TAG` changes at restart, so full IDs never actually repeat across processes-over-time.
- `PROCESS_TAG` is not cryptographic — two processes starting at the same time with the same entropy seed could theoretically collide. With 32 bits of randomness and typical process counts this is negligible, but anyone running millions of processes should override with `randomUUID()`-based IDs.
