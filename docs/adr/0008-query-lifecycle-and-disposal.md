...
- The library has many user-facing types with lifetimes — `ReservedConn`, `Transaction`, the `Savepoint` handles a transaction hands out, `PreparedStatement`, and `Query` itself. They should expose one cleanup verb (`.dispose()`) consumers can rely on, with named domain methods where they carry distinct meaning.
...
- **Early termination via `for await`** — breaking, returning, or throwing inside the loop invokes the iterator's `return()` method, which asks the driver to cancel the request. The connection returns to the pool immediately. The terminal's tracingChannel `asyncEnd` fires with `reason: 'cancelled'`.
- **`.columns()`-only consumption** — calling `.columns()` without a subsequent row-consuming terminal leaves the stream held by driver backpressure mid-response (see [ADR-0007](0007-query-result-presentation.md) for the backpressure model). `.dispose()` on the Query (explicit, or via `await using`) asks the driver to cancel the request, and the connection returns to the pool clean. This is the explicit "the caller asked for the shape but not the rows" path. The terminal's `asyncEnd` fires with `reason: 'cancelled'`.
- **Explicit cancellation via `.cancel()` or `AbortSignal`** — `Query.cancel()` and aborting the signal both issue the driver-level cancel; the connection returns to the pool cleanly, and the next acquirer picks it up like any other release. No connection is wasted on the happy cancel path. The pool adapter only destroys a connection when the cancel itself leaves it in an unrecoverable state — a validate-failure concern owned by the adapter, not the query lifecycle. The terminal's diagnostics outcome depends on what fired the cancel: an `AbortSignal.timeout()` deadline (`signal.reason.name === 'TimeoutError'`) or `defaultTimeout` produces a `TimeoutError` on the tracingChannel `error` channel; everything else (`Query.cancel()`, `controller.abort()` without a timeout reason) produces `asyncEnd` with `reason: 'cancelled'`. The exact cancel mechanism is the driver's concern — the kernel asks the `Connection` to cancel and the driver decides how to communicate that to the server.
...
2. **A terminal has fired** — ask the driver to cancel the request. The connection returns to the pool clean. Once the stream has terminated, `meta()` returns with `completed: false` and whatever trailer data (info / print / envChanges / `rowsAffectedPerStatement` so far) arrived before the cancel. This holds regardless of whether the stream was flowing (a consuming terminal was reading rows), held by backpressure (`.columns()` resolved with no row consumer), or in between.
...
The implication: returning an unsettled promise from inside an `await using` scope cancels its in-flight work — `return q.all()` rejects with `AbortError` at the caller; `return await q.all()` is the form that survives. Returning the `Query` itself from such a scope leaves the caller with a disposed object whose next terminal throws `TypeError`. These are consequences of the rule, not separate decisions; working patterns are in the usage docs.
...
| `Savepoint` handle (from `tx.savepoint()`) | **release** the mark (drop it from the stack, **keeping its work**) if not already rolled back or released — savepoints are kept-by-default bookmarks; roll back to one explicitly to discard work ([ADR-0006](0006-queryable-api.md), "Savepoints") | `.rollback()`, `.release()` |
...
**On savepoints — bookmarks, not transactions.** Within a transaction, partial rollback uses savepoints the transaction records on a stack ([ADR-0006](0006-queryable-api.md), "Savepoints"). A savepoint is not a transaction: it has no commit. Its handle has two settle verbs — `rollback()` (discard the work since the mark; `ROLLBACK TRANSACTION <name>` on the wire) and `release()` (drop the mark, **keep** its work; application-layer only, as SQL Server has no `RELEASE SAVEPOINT` verb). `await using sp`'s disposal **releases** if you haven't already settled — savepoints are *kept by default*, so the safe default is to drop the bookmark and leave its work in the transaction, not to roll it back. This deliberately differs from a `Transaction`, whose disposal *rolls back* if not committed: a transaction is a unit of work you must commit to keep, a savepoint is a bookmark whose work is already in the transaction. The `.dispose()` table calls this out so the savepoint row isn't mistaken for the transaction's rollback-on-dispose.

**The transaction settles savepoints wholesale.** `tx.commit()` / `tx.rollback()` is a *single* wire operation that commits or discards the entire transaction including every open savepoint; both clear the savepoint stack, leaving any outstanding handle spent. There is no per-savepoint settlement ceremony — in particular `tx.rollback()` does not first issue a cascade of `ROLLBACK TRANSACTION <mark>` round-trips (a single `ROLLBACK` suffices). A spent savepoint handle (its mark rolled past, or the transaction settled) follows the usual disposable split: an **explicit** `.rollback()` throws `TypeError`, while `await using` disposal is a **forgiving no-op** (convention 1). This is also why savepoints are transaction-managed rather than modelled as nested transactions you settle individually: a lost savepoint handle can never block the transaction's commit, and `tx.rollbackSavepoint()` lets a caller roll back without holding the handle at all.
...
- Savepoints are kept-by-default bookmarks the transaction owns, not transactions: `tx.savepoint()`'s handle has `rollback()` (discard work) and `release()` (drop the mark, keep work — application-layer, since TDS has no `RELEASE SAVEPOINT` verb), and its `await using` disposal *releases* (keeps the work). The transaction settles all savepoints wholesale on commit/rollback. The disposal notes call this out so users do not assume a savepoint rolls back on dispose the way a `Transaction` does, or expect Postgres / SQLite release parity.
