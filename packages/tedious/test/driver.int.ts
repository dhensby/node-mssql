...
describe('tediousDriver — sql.acquire (integration)', () => {
	...
	test('client.close() waits for a held ReservedConn to be released (graceful drain)', async () => {
		// Graceful drain must not tear down the connection while a holder
		// still owns it. Proof is by ordering: the `release` marker is
		// pushed BEFORE close() resolves, so close() genuinely waited.
		const client = makeClient(requireIntegrationConfig());
		await client.connect();
		let cleanedUp = false;
		try {
			const conn = await client.sql.acquire();
			await conn`SELECT 1`.run();   // prove the held connection is live
			...
			// Happy path already closed it; this covers an early-failure exit.
			if (!cleanedUp) await client.close().catch(() => { /* */ });
		}
	});
});
...
// ─── Savepoints — transaction-managed bookmarks (integration) ─────────────
//
// `tx.savepoint()` → `SAVE TRANSACTION`; `sp.rollback()` →
// `ROLLBACK TRANSACTION <name>` (discard); `sp.release()` / disposal →
// drop the mark keeping its work (no wire op). The transaction owns the
// stack; rolling back to an earlier mark discards later ones. Exercised
// against a real SQL Server with a permanent table so the persisted /
// discarded outcomes are unambiguous.

describe('tediousDriver — savepoints (integration)', () => {
	test('rolling back to a savepoint discards work since it; commit persists the rest', async () => {
		const client = makeClient(requireIntegrationConfig());
		await client.connect();
		try {
			const tableName = `dbo.t_${Math.random().toString(36).slice(2, 10)}`;
			await client.sql.unsafe(`CREATE TABLE ${tableName} (id INT)`).run();
			try {
				const tx = await client.sql.transaction();
				try {
					await tx.unsafe(`INSERT INTO ${tableName} (id) VALUES (1)`).run();
					...
					await sp.rollback();   // discard id 2
	...
	test('releasing a savepoint keeps its work; the commit persists it', async () => {
		const client = makeClient(requireIntegrationConfig());
		await client.connect();
		try {
			const tableName = `dbo.t_${Math.random().toString(36).slice(2, 10)}`;
			...
			try {
				...
				try {
					const sp = await tx.savepoint();
					await tx.unsafe(`INSERT INTO ${tableName} (id) VALUES (10)`).run();
					await sp.release();   // keep the work, drop the mark
					await tx.commit();
				} catch (err) {
					await tx.rollback();
					throw err;
	...
	test('await using savepoint disposal keeps its work (release, not rollback)', async () => {
		const client = makeClient(requireIntegrationConfig());
		await client.connect();
		try {
			const tableName = `dbo.t_${Math.random().toString(36).slice(2, 10)}`;
				...
				try {
					{
						...
						await tx.unsafe(`INSERT INTO ${tableName} (id) VALUES (7)`).run();
						void sp;
						// fall off → disposal releases (keeps id 7)
					}
					await tx.commit();
				} catch (err) {
					await tx.rollback();
	...
	test('rolling back to an earlier savepoint discards every later mark', async () => {
		const client = makeClient(requireIntegrationConfig());
		await client.connect();
		try {
			const tableName = `dbo.t_${Math.random().toString(36).slice(2, 10)}`;
			...
			try {
				...
				try {
					...
					await tx.unsafe(`INSERT INTO ${tableName} (id) VALUES (2)`).run();
					await tx.savepoint();   // sp2 — created after sp1
					await tx.unsafe(`INSERT INTO ${tableName} (id) VALUES (3)`).run();
					await sp1.rollback();   // ROLLBACK TO sp1 — discards ids 2 and 3
					await tx.commit();
				} catch (err) {
					await tx.rollback();
					throw err;
	...
	test('tx.rollbackSavepoint() rolls back to the most-recent mark without a handle', async () => {
		const client = makeClient(requireIntegrationConfig());
		await client.connect();
		try {
			const tableName = `dbo.t_${Math.random().toString(36).slice(2, 10)}`;
			...
			try {
				...
				try {
					await tx.unsafe(`INSERT INTO ${tableName} (id) VALUES (1)`).run();
					await tx.savepoint();   // handle intentionally not held
					await tx.unsafe(`INSERT INTO ${tableName} (id) VALUES (2)`).run();
					await tx.rollbackSavepoint();   // roll back to the most-recent mark → discards id 2
					await tx.commit();
				} catch (err) {
					await tx.rollback();
					throw err;
	...
	test('parallel savepoint() creation serialises into ordered, distinct marks (no EREQINPROG)', async () => {
		const client = makeClient(requireIntegrationConfig());
		await client.connect();
		try {
			const tableName = `dbo.t_${Math.random().toString(36).slice(2, 10)}`;
