...
2. The terminal rejects with an `AbortError` (`name: 'AbortError'`, matching `fetch` / Node convention) — or `TimeoutError` if `signal.reason` was a `'TimeoutError'`-named `DOMException` from `AbortSignal.timeout()`.
3. Core publishes the outcome on the terminal's tracingChannel: `error` for `TimeoutError`, `asyncEnd` with `reason: 'cancelled'` for `AbortError`.
...
- **User `.signal()` aborts**: driver cancel, terminal rejects with `AbortError` or `TimeoutError` per `signal.reason.name`. For `AbortError` (the consumer's intentional cancel) the terminal's `asyncEnd` fires with `reason: 'cancelled'`; for `TimeoutError` (the consumer's `AbortSignal.timeout()` deadline) the `error` channel fires. The user's signal remains in force for the entire request lifetime, including during streaming.
...
- Diagnostics subscribers see cancellations and errors split across two surfaces of the terminal's tracingChannel: `asyncEnd` with `reason: 'cancelled'` for user-explicit cancels (`Query.cancel()`, `for await` `break` / `return` / `throw`, `controller.abort()` without a timeout reason), and the `error` channel for timeouts (`defaultTimeout` firing or a consumer signal whose `signal.reason.name === 'TimeoutError'`) and any genuine errors. Operational dashboards page on the `error` channel; clean cancellations on `asyncEnd` are recorded but not alerted.
- The categorisation matches the catch-site error class one-for-one — `TimeoutError` always reaches the `error` channel; `AbortError` always reaches `asyncEnd`. The original `signal.reason` is preserved on the thrown error's `.cause` (and on `asyncEnd`'s `error.cause` for cancellations), so consumers walking the cause chain see the raw `DOMException` produced by `AbortSignal.timeout()` / `controller.abort()` — no separate diagnostics-channel field needed for it.
